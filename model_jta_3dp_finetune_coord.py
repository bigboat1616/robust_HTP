...
class TransMotion3DP(nn.Module):
    ...
    def forward(self, tgt, padding_mask, metamask=None):
        ...
        tgt_3dpose_original = self.fc_in_3dpose(tgt_3dpose_9frames) # (B, 9, N, 22,64)
        ...
        tgt_3dpose_stgcn_input = tgt_3dpose_9frames.permute(0, 2, 1, 3, 4).contiguous()  # (B, N, 9, 22, 64)
        tgt_3dpose_stgcn_input = tgt_3dpose_stgcn_input.view(BN, 9, 22, 3)  # (B*N, 9, 22, 3)
        # ST-GCNは(N, C, T, V)形式を期待: (B*N, 3, 9, 22)
        ...
        tgt_3dpose = tgt_3dpose + tgt_3dpose_original
