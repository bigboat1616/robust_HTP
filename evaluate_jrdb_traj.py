...
def evaluate_ade_fde(model, dataloader, bs, config, logger, return_all=False, bar_prefix=""):
    ...
    for i, batch in enumerate(dataloader):
        ...
        in_joints = in_joints.cpu()  # 観測データ用に追加
        ...
        for k in range(len(out_joints)):
            # パディングマスクをチェック
            if padding_mask[k].sum() == padding_mask[k].shape[0]:  # 完全にパディングされている場合
                continue

            person_out_joints = out_joints[k,:,0:1]  # Ground Truth
            person_pred_joints = pred_joints[k,:,0:1]  # 予測
            person_in_joints = in_joints[k,:,0:1]  # 観測データ
