...
class TransMotionTraj(nn.Module):
    ...
    def apply_frequency_schedule(self, freq_domain, num_freqs=None):
        """周波数成分のスケジューリング
        Args:
            freq_domain: FFT後の周波数領域データ [B, F, N, 2]
            num_freqs: 使用する周波数成分の数（Noneの場合は全て使用）
                1: DC成分のみ
                2: DC + 最低周波数対（F[1]とF[-1]）
                3: DC + 低周波2対（F[1:3]とF[-3:-1]）
                4: DC + 低周波3対（F[1:4]とF[-4:-1]）
                5: 全ての周波数（F[0:5]とF[-4:]）
        """
        if num_freqs is not None:
            n = freq_domain.size(1)  # F = 9
            half_n = (n - 1) // 2  # (9-1)/2 = 4
            
            # 周波数マスクの生成
            mask = torch.zeros_like(freq_domain, device=freq_domain.device)
            # DC成分（F[0]）は常に保持
            mask[:, 0] = 1.0
            
            if num_freqs > 1:
                # 低周波から順に周波数対を追加
                for i in range(1, min(num_freqs - 1, half_n + 1)):
                    mask[:, i] = 1.0      # 正の周波数側（低→高）
                    mask[:, -i] = 1.0     # 負の周波数側（高→低）の対応する成分
            
            # マスクを適用
            freq_domain = freq_domain * mask
            
        return freq_domain

    def forward(self, tgt, padding_mask, metamask=None, num_freqs=None):
        ...
        # 周波数スケジューリングを適用
        freq_domain = self.apply_frequency_schedule(freq_domain, num_freqs)
